...
def tree_grow(x: np.ndarray, y: np.ndarray, nmin: int, minleaf: int, nfeat: int) -> RenderTree:
    ...
    global tree_iter
    tree_iter = 1
    node_list = [Node(f'root {tree_iter - 1}', features=x, label=y, attribute_index=None, best_split=None)]
    root = node_list[0]
            ...
            if current_node.features.shape[0] >= nmin:
                ...
                for idx, feat in enumerate(sub_feat):
                    ...
                    if len(np.unique(feat)) > 1:
                        split_value = compute_best_split(feat, current_node.label)
                        temp_Lchild, temp_Rchild = split(current_node, rng_feat_idx[idx], split_value, minleaf)

                        # Impurity reduction is only computed if the split is acceptable
                        if temp_Lchild is not None and temp_Rchild is not None:
                            new_imp_reduc = impurity_reduction(current_node, temp_Lchild, temp_Rchild)
                ...
                if candidate_children[0] is not None and candidate_children[1] is not None:
                    ...
                    + '\n' + '0:' + str(len(current_node.label==0)) + ' | 1:' + str(len(current_node.label==1))
                    node_list.extend([candidate_children[0], candidate_children[1]])

                    tree_iter += 1
    return RenderTree(root)

def tree_pred(x: np.ndarray, tr: RenderTree) -> list:
    """
...
def tree_grow_b(x: np.ndarray, y: np.ndarray, nmin: int, minleaf: int, nfeat: int, m: int) -> list:
    ...
    for idx in range(m):
        ...
        tree_list.append(tree_grow(X_new, Y_new, nmin, minleaf, nfeat))
    return tree_list

def tree_pred_b(x: np.ndarray, tree_list: list) -> list:
    """
    ...
    for idx, tree in enumerate(tree_list):
        y_pred_b.append(tree_pred(x, tree))
...
def impurity_reduction(parent: Node, left_child: Node, right_child: Node) -> float:
    """ Computes the impurity reduction of the split that generates Nodes "left_child" and "right_child" from the Node "parent" , based on gini-index. 
            ...
            """
    return impurity(parent.label) - impurity(left_child.label) * len(left_child.features) / len(
        parent.features) - impurity(right_child.label) * len(right_child.features) / len(parent.features)

def split(parent: Node, feat_idx: int, pos, minleaf) -> tuple:  # add the contraint minleaf
    ...
    if len(parent.features[parent.features[:, feat_idx] >= pos]) < minleaf or len(
        ...
        Lchild = Node(f'L {tree_iter}', features=parent.features[parent.features[:, feat_idx] >= pos],
                        label=parent.label[parent.features[:, feat_idx] >= pos])
        Rchild = Node(f'R {tree_iter}', features=parent.features[parent.features[:, feat_idx] < pos],
                        label=parent.label[parent.features[:, feat_idx] < pos])
...
credit_data = genfromtxt('C:\\Users\\giada\\Documents\\GitHub\\Data_Mining\\credit.txt', delimiter=',', skip_header=True)
...
pima_data = genfromtxt('C:\\Users\\giada\\Documents\\GitHub\\Data_Mining\\pima.txt', delimiter=',')
